...
namespace ShareMemRPCLite
{
    internal sealed class UnitShareMem : IDisposable
    {
        ...
        public int WriteToShare(string value, int port = -1, bool canInvokeGVision = true)
        {
            ...
            SOneCamShareMem share = null;

            lock (runShareLock)
            {
                if (port >= 0)
                {
                    runShareDic.TryGetValue(port, out share);
                }
            }

            if (share != null)
            {
                accessor = share.RunAccessor;
                doHandle = share.RunDoHandle;
                finishHandle = share.RunFinishHandle;
                writeLength = share.StrSize;
            }
            else
            {
        ...
        public GVisionRtnCode WaitWriteOK(uint timeMs = 200, int port = -1)
        {
            ...
            if (port >= 0 && share == null)
            {
                return GVisionRtnCode.NoFindGVision;
            }

            uint waitResult = port >= 0
                ? WaitForSingleObject(share.RunFinishHandle, timeMs)
            ...
            try
            {
                if (share != null)
                {
                    lock (share.RunSyncRoot)
                    {
                        ...
                        accessor.ReadArray(startPosition, shareBytes, 0, shareBytes.Length);
                        value.ConnStr = Encoding.UTF8.GetString(shareBytes).Trim('\0');
                        return 0;
                    }
                }
            ...
            lock (runShareLock)
            {
                foreach (KeyValuePair<int, Tuple<int, int, int>> item in tempCamMsg)
                {
                    ...
                    else if (runShareDic.ContainsKey(imageBindCam))
                    {
                        ...
                        runShareDic[cam] = new SOneCamShareMem(
                            strLength,
                            cam,
                            source.ImgSize,
                            source.ImageMappedFile,
                            source.ImageAccessor,
                            source.ImageSyncRoot);
                    }
                    else
                    {
                        runShareDic[cam] = new SOneCamShareMem(strLength, cam, imageSize);
        ...
        private void ListenImageLoop(int port)
        {
            while (isListenImage)
            {
                ...
                if (share.ImageFinishHandle == IntPtr.Zero)
                {
                    Thread.Sleep(20);
                    continue;
                }
        ...
        private static Bitmap TryReadBitmap(SOneCamShareMem share)
        {
            ...
            try
            {
                lock (share.ImageSyncRoot)
                {
                    MemoryMappedViewAccessor imageAccessor = share.ImageAccessor;
                    if (imageAccessor == null)
                    {
                        return null;
                    }

                    int imgWH = imageAccessor.ReadInt32(0);
                    if (imgWH < 1)
                    {
                        return null;
                    }
                    ...
                    imageAccessor.ReadArray(4, bytes, 0, bytes.Length);
