...
namespace ShareMemRPCLite
{
    ...
    internal sealed class SOneCamShareMem : IDisposable
    {
        ...
        private readonly bool ownsImageMemory;
        ...
        public SOneCamShareMem(int strSize, int camId, int imageSize = -1,
            ...
            object imageSyncRoot = null)
        {
            CamID = camId;
            StrSize = strSize;
            ImgSize = imageSize;
            RunSyncRoot = new object();
            ImageSyncRoot = imageSyncRoot ?? new object();
            ...
            if (imageMap == null && imageSize > 0)
            {
                ...
                ownsImageMemory = true;
            }
            else
            {
                ImageMappedFile = imageMap;
                ImageAccessor = imageAccessor;
                ownsImageMemory = false;
            }
        ...
        public void Dispose()
        {
            ...
            lock (ImageSyncRoot)
            {
                if (ownsImageMemory && ImageAccessor != null)
                {
                    ImageAccessor.Dispose();
                }
                ImageAccessor = null;

                if (ownsImageMemory && ImageMappedFile != null)
                {
