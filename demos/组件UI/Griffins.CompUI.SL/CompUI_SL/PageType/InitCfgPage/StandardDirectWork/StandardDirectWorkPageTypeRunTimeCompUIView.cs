...
{
    internal class StandardDirectWorkPageTypeRunTimeCompUIView : IPageTypeRunTimeCompUIView
    {
        private ICompUIRunTimeCallBack callBack;

        private event EventHandler afterModified;

        private StandardDirectWorkCompUIView view;

        private readonly StandardDirectWorkCompUIViewModel viewModel;

        private StandardDirectWorkCompUIModel‌ standardDirectWorkCompUIModel‌;
        ...
        public StandardDirectWorkPageTypeRunTimeCompUIView(ICompUIRunTimeCallBack callBack)
        {
        ...
        /// 设置数据
        /// </summary>
        /// <param name="standardDirectWorkCompUIModel‌">数据源</param>
        public void SetData(StandardDirectWorkCompUIModel‌ standardDirectWorkCompUIModel‌)
        {
            this.standardDirectWorkCompUIModel = standardDirectWorkCompUIModel ?? new();
        ...
        {
            if (e.PropertyName == nameof(viewModel.ScannerEnabled)
                || e.PropertyName == nameof(viewModel.BarcodeCheckChar)
                || e.PropertyName == nameof(viewModel.BarcodeType)
                || e.PropertyName == nameof(viewModel.ScannerType)
                || e.PropertyName == nameof(viewModel.HeightDelayMs)
                || e.PropertyName == nameof(viewModel.LiftFixedTimeMs)
                || e.PropertyName == nameof(viewModel.LiftReleaseFixedTimeMs)
                || e.PropertyName == nameof(viewModel.TempDetectEnabled)
                || e.PropertyName == nameof(viewModel.TempWorkEnabled)
                || e.PropertyName == nameof(viewModel.TempPreheatEnabled)
                || e.PropertyName == nameof(viewModel.CustomStationSensors))
            {
