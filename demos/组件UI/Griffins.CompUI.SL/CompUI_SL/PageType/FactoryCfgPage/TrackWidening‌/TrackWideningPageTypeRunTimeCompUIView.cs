...
namespace Griffins.CompUI.SL.FactoryCfgPage
{
    internal class TrackWideningPageTypeRunTimeCompUIView : IPageTypeRunTimeCompUIView
    {
        ...
        public TrackWideningCompUIModel‌ GetData()
        {
            ...
            this.trackWideningCompUIModel‌.LeftTrackMovable = viewModel.LeftTrackMovable;
            this.trackWideningCompUIModel‌.RightTrackMovable = viewModel.RightTrackMovable;
            this.trackWideningCompUIModel‌.TrackID = viewModel.TrackID;
            this.trackWideningCompUIModel‌.TrackLocation = viewModel.TrackLocation;
        ...
        {
            if (e.PropertyName == nameof(viewModel.LeftTrackMovable))
            {
                afterModified?.Invoke(sender, null);
            }
            else if (e.PropertyName == nameof(viewModel.RightTrackMovable))
            {
                afterModified?.Invoke(sender, null);
            }
            else if (e.PropertyName == nameof(viewModel.TrackID))
            {
                afterModified?.Invoke(sender, null);
            }
            else if (e.PropertyName == nameof(viewModel.TrackLocation))
            {
                afterModified?.Invoke(sender, null);
            }
        }
