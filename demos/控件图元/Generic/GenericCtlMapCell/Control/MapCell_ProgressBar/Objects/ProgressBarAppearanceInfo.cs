...
namespace GKG.Map.MapCell.Generic.ProgressBar
{
    ...
    public class ProgressBarAppearanceInfo : MiniReactiveObject, IJsonValueConvert, IMPPropObjectValue
    {
        public static readonly ProgressBarAppearanceInfo Default = new ProgressBarAppearanceInfo(1.0, 1, 1, 1, 1);
        public static readonly Guid Object_ID = new Guid("{58F8A3E8-41D3-4B34-9F32-9C2B3F1C8A12}");
        ...
        public ProgressBarAppearanceInfo()
            : this(1.0, 1, 1, 1, 1)
        {
        }

        public ProgressBarAppearanceInfo(double opacity, double borderLeft, double borderTop, double borderRight, double borderBottom)
        {
            Opacity = opacity;
        ...
        void IMPPropObjectValue.PopulateFromBaseValue(GriffinsBaseValue baseValue)
        {
            if (baseValue != null && baseValue.Val != null)
            {
                if (!(baseValue.Val is ObjectValue_Json))
                    throw new Exception("对象值不是ProgressBarAppearanceInfo转换的");
