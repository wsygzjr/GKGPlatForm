...
namespace GKG.Map.MapCell.Generic.RadioButton
{
    ...
    public class RadioButtonLayoutInfo : MiniReactiveObject, IJsonValueConvert, IMPPropObjectValue
    {
        #region 静态字段

        public static readonly RadioButtonLayoutInfo Default = new RadioButtonLayoutInfo();
        public static readonly Guid Object_ID = new Guid("{C1B2C3D4-3333-4AC8-BF66-281412CDE203}");
        ...
        public RadioButtonLayoutInfo()
            : this(HorizontalAlignType.Stretch, VerticalAlignType.Stretch, 0, 0, 0, 0, 0, 0, 0, 0)
        {
        }

        public RadioButtonLayoutInfo(HorizontalAlignType horizontalAlign, VerticalAlignType verticalAlign,
        ...
        void IMPPropObjectValue.PopulateFromBaseValue(GriffinsBaseValue baseValue)
        {
            if (baseValue != null && baseValue.Val != null)
            {
                if (!(baseValue.Val is ObjectValue_Json))
                    throw new Exception("对象值不是RadioButtonLayoutInfo转换的");
