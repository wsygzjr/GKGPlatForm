...
namespace GKG.Map.MapCell.Generic.RadioButton
{
    ...
    public class RadioButtonAppearanceInfo : MiniReactiveObject, IJsonValueConvert, IMPPropObjectValue
    {
        #region 静态字段

        public static readonly RadioButtonAppearanceInfo Default = new RadioButtonAppearanceInfo(1.0, 1, 1, 1, 1);
        public static readonly Guid Object_ID = new Guid("{C1B2C3D4-2222-4AC8-BF66-281412CDE202}");
        ...
        public RadioButtonAppearanceInfo()
            : this(1.0, 1, 1, 1, 1)
        {
        }

        public RadioButtonAppearanceInfo(double opacity, double borderLeft, double borderTop, double borderRight, double borderBottom)
        {
            Opacity = opacity;
        ...
        void IMPPropObjectValue.PopulateFromBaseValue(GriffinsBaseValue baseValue)
        {
            if (baseValue != null && baseValue.Val != null)
            {
                if (!(baseValue.Val is ObjectValue_Json))
                    throw new Exception("对象值不是RadioButtonAppearanceInfo转换的");
