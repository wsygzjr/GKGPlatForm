...
namespace GKG.Map.MapCell.Generic.RadioButton
{
    ...
    public class RadioButtonBrushInfo : MiniReactiveObject, IJsonValueConvert, IMPPropObjectValue
    {
        #region 静态字段

        public static readonly RadioButtonBrushInfo Default = new RadioButtonBrushInfo(
            Color.FromArgb(0, 255, 255, 255),
            Color.FromArgb(255, 100, 100, 100),
            Colors.Black
        );

        public static readonly Guid Object_ID = new Guid("{C1B2C3D4-1111-4AC8-BF66-281412CDE201}");
        ...
        public RadioButtonBrushInfo()
            : this(Color.FromArgb(0, 255, 255, 255), Color.FromArgb(255, 100, 100, 100), Colors.Black)
        {
        }

        public RadioButtonBrushInfo(Color backColor, Color borderColor, Color foreColor)
        {
        ...
        void IMPPropObjectValue.PopulateFromBaseValue(GriffinsBaseValue baseValue)
        {
            if (baseValue != null && baseValue.Val != null)
            {
                if (!(baseValue.Val is ObjectValue_Json))
                    throw new Exception("对象值不是RadioButtonBrushInfo转换的");
