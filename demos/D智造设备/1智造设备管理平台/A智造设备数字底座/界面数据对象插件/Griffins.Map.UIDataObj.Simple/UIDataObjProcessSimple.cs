...
namespace Griffins.Map.UIDataObj.ImeIot
{
	...
	internal class UIDataObjProcessSimple : GriffinsPluginMngClass, IUIDataObjProcess
	{
		...
		//			uIDataObjPropValue.Alias = this.cfgInfo.YS_Alias.ToString();
		//			uIDataObjPropValue.PropertyID = MPPropertyID.Parse("Prop1");
		//			uIDataObjPropValue.Value = uIDataObjPropValue.Value;//需要将自定义界面对象属性值转换为模组的
		//			//uIDataObjPropValue.Value = new GriffinsBaseValue("5");//需要将自定义界面对象属性值转换为模组的
		...
		/// <summary>
		/// 获取指定实例别名的组件界面数据对象属性路径值
		/// </summary>
		/// <param name="projectID">项目ID，如果用于网关，该项无效</param>
		/// <param name="alias">组件界面数据对象实例别名</param>
		/// <param name="objKind">界面数据对象种类</param>
		/// <param name="category">界面数据对象类别</param>
		/// <returns>指定实例别名的组件界面数据对象属性路径值列表</returns>
		...
		/// <summary>
		/// 获取指定实例别名列表的组件界面数据对象属性路径值
		/// </summary>
		/// <param name="projectID">项目ID，如果用于网关，该项无效</param>
		/// <param name="aliases">组件界面数据对象实例别名列表</param>
		/// <param name="objKind">界面数据对象种类</param>
		/// <param name="category">界面数据对象类别</param>
		/// <returns>指定实例别名列表的组件界面数据对象属性路径值列表</returns>
		...
		{
			return new UIDataObjPropPathValueList();
		}
		/// <summary>
		/// 获取指定实例别名的组件界面数据对象属性路径当前值
		/// </summary>
		/// <param name="projectID">项目ID，如果用于网关，该项无效</param>
		/// <param name="alias">组件界面数据对象实例别名</param>
		/// <param name="objInstPropPath">组件界面数据对象属性路径ID</param>
		/// <param name="objKind">界面数据对象种类</param>
		/// <param name="category">界面数据对象类别</param>
		/// <returns>指定实例别名的组件界面数据对象属性路径当前值</returns>
		...
		/// <param name="projectID">项目ID，如果用于网关，该项无效</param>
		/// <param name="alias">组件界面数据对象实例别名</param>
		/// <param name="objInstPropPaths">组件界面数据对象属性路径ID列表</param>
		/// <param name="objKind">界面数据对象种类</param>
		/// <param name="category">界面数据对象类别</param>
		/// <returns>指定实例别名一组属性路径当前值列表</returns>
		...
		/// </summary>
		/// <param name="projectID">项目ID，如果用于网关，该项无效</param>
		/// <param name="uIDataObjPropPathValue">组件界面数据对象属性路径值</param>
		/// <param name="objKind">界面数据对象种类</param>
		/// <param name="category">界面数据对象类别</param>
		...
		/// </summary>
		/// <param name="projectID">项目ID，如果用于网关，该项无效</param>
		/// <param name="uIDataObjPropPathValues">组件界面数据对象属性路径值列表</param>
		/// <param name="objKind">界面数据对象种类</param>
		/// <param name="category">界面数据对象类别</param>
		...
		{
            UIDataObjPropPathValueList ysUIDataObjPropValues = new UIDataObjPropPathValueList();
			foreach (UIDataObjPropPathValue uIDataObjPropValue in uIDataObjPropPathValues)
			{
				if (uIDataObjPropValue.ObjInstPropPath == ObjInstPropPath.Parse("Param1"))
				{
					uIDataObjPropValue.Alias = this.cfgInfo.SXL_Alias.ToString();
					uIDataObjPropValue.ObjInstPropPath = ObjInstPropPath.Parse("TestParam1");
					uIDataObjPropValue.Value = uIDataObjPropValue.Value;//需要将自定义界面对象属性值转换为模组的
					ysUIDataObjPropValues.Add(uIDataObjPropValue);
				}
			}
			...
			if (ysUIDataObjPropValues.Count > 0)
			{
				iImeCompUIDataObjPropValRW.SetUIDataObjPropPathValues(projectID, ysUIDataObjPropValues, UIDataObjProcessSimpleCfgInfo.SXL_MPKind, ImeIOTConst.UIDataObjCategory_MM);
			}
        ...
        private void doMMUIDataObjPropValChangedInfo(GriffinsInfoKindID infoKind, string infoNo, InformInfoBase info)
		{
			...
			if (informInfo.MMAlias == this.cfgInfo.SXL_Alias)
			{
				if (informInfo.ObjInstPropPath == ObjInstPropPath.Parse("TestParam1"))
				{
					var uiDataObjPropValue = new UIDataObjPropValue()
					{
						...
						//将模组的界面数据对象属性值ID，转换为自定义界面数据对象种类的属性值ID
						PropertyID = MPPropertyID.Parse("Param1"),
						//将模组的界面数据对象属性值转换为自定义界面数据对象种类的属性值
						Value = convert(informInfo.ObjInstPropPath,informInfo.Val)
					};
		...
		private GriffinsBaseValue convert(ObjInstPropPath objInstPropPath, GriffinsBaseValue griffinsBaseValue)
		{
		...
		private class UIDataObjProcessSimpleCfgInfo
		{
            ...
            /// 印刷机械模组界面数据对象种类
            /// </summary>
            public static readonly ManagingPointKind SXL_MPKind = ManagingPointKind.Parse("test_bzsxl");
            /// <summary>
            /// 印刷机械模组实例别名
            /// </summary>
			...
			/// 上下料械模组实例别名
			/// </summary>
			public MMAlias SXL_Alias { get; set; } = new MMAlias("sxl");
